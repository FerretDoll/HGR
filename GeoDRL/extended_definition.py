...
class ExtendedDefinition(BasicDefinition):
    ...
    def definePolygon(self, points):
        ...
        if l == 3:
            # dictionary order
            facts(self.Triangle, sorted(points))
        elif l == 4:
            facts(self.Quadrilateral, sort_points(points))
        elif l == 5:
            facts(self.Pentagon, sort_points(points))
    ...
    def newLineSymbol(self, line):
        ...
        # sym = Symbol("LengthOf(Line({},{}))".format(*[str(ch) for ch in line]))
    ...
    def newAngleSymbol(self, angle):
        ...
        # sym = Symbol("MeasureOf(Angle({},{},{}))".format(*[str(ch) for ch in angle]))
    ...
    def newArcSymbol(self, arc):
        ...
        # sym = Symbol("MeasureOf(Arc({},{},{}))".format(*[str(ch) for ch in arc]))
        sym = Symbol("arc_" + ''.join([str(ch) for ch in arc]))
        self.defineArc(arc[0], arc[1], arc[2], sym)
        return sym
    
    def newAreaSymbol(self, expr):
        sym = Symbol("AreaOf({})".format(expr))
        return sym
    ...
    def lineEqual(self, line1, line2):
        ...
        self.PutIntoEqualLineSet(line1, line2)
    ...
    def angleEqual(self, angle1, angle2):
        ...
        self.PutIntoEqualAngleSet(angle1, angle2)
    ...
    def arcEqual(self, arc1, arc2):
        ...
        self.PutIntoEqualArcSet(arc1, arc2)
    ...
    def defineSimilarTriangle(self, tri1, tri2):
        ...
        facts(self.SimilarTriangle, (*tri1, *tri2))
    ...
    def defineCongruentTriangle(self, tri1, tri2):
        self.definePolygon(tri1)
        self.definePolygon(tri2)
        facts(self.CongruentTriangle, (*tri1, *tri2))
        # for ch in permutations([0, 1, 2]):
        #     facts(self.CongruentTriangle, (tri1[ch[0]], tri1[ch[1]], tri1[ch[2]], tri2[ch[0]], tri2[ch[1]], tri2[ch[2]]))
        #     facts(self.CongruentTriangle, (tri2[ch[0]], tri2[ch[1]], tri2[ch[2]], tri1[ch[0]], tri1[ch[1]], tri1[ch[2]]))
        # for ch in range(3):
    ...
    def defineSimilarPolygon(self, poly1, poly2):
        assert len(poly1) == len(poly2)
        self.definePolygon(poly1)
        self.definePolygon(poly2)
        
        facts(self.SimilarPolygon, (*poly1, *poly2))
    ...
    def set_angle_sum(self):
        ...
        for angle in angles:
            if angle[3] == 180:
                points = set(self.find_all_lines_for_point(angle[1])) - \
                         set(self.find_all_points_on_line((angle[0], angle[2])))
                for point in points:
                    val = self.find_angle_measure((point, angle[1], angle[2]))
                    if val != []:
                        self.define_angle_measure(angle[0], angle[1], point, 180 - val[0])
